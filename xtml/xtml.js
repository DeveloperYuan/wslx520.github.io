...
<div id="peoples">\
    ...
    <p>\
        ...
        name:{{index}},{{people.name}}, family:{{pf.join(\',\')}}\
    </p>\
    {{/~}}\
</div>\
\
...
function compile(str) {
	return new Function('it', 'var out=\'\';' + newend 
        ...
        //     } else {
        //         ma = 'for (var arr_'+(objId+=1)+'=' + obj + ','+ key +' = 0, '+ value +', arr_len_'+objId+' = '+ obj +'.length; '+key+'<arr_len_'+objId+';'+key+'++) {'+value+' = arr_'+objId+'['+key+']; ';
        //     }
        //     console.log(varReplace(ma), newstart)
        //     return newstart+ varReplace(ma) + newend;
        // })
        .replace(blockReg, function (m, $1, first) {
            // console.log(m,'---------', $1,'---------', $2);
            $1 = trim($1);
            first = $1.charAt(0);
            if(first === '~') {
            ...
            console.log($1,varReplace($1))
            return joinstart + varReplace($1) + joinend;
        })
        // 去除空的 out += ''这样的字符串
        .replace(blankStrReg,'')
        + newstart
